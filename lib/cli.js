#!/usr/bin/env node
var cli, colors, cpuThreshold, d, error, findOffenders, fs, generateDeltaText, generateGcText, generateHiddenStackHint, hiddenStackWarningDisplayed, meow, obj, offenders, parseThreadDumps, parseTop, parsedThreadDumps, parsedTop, pid, proc, processes, ref, ref1, thread, threadLengthDisplay, timestamp,
...
generateHiddenStackHint = function(threadLengthDisplay, threadLength) {
  var linesHidden;
      ...
      if (thread != null) {
        thread.forEach(function(stackLine, i) {
          if (i <= +threadLengthDisplay) {
            return console.log(("\t" + stackLine + (generateGcText(stackLine))).cyan);
          } else {
            if (!hiddenStackWarningDisplayed) {
              console.log("\t... " + (generateHiddenStackHint(+threadLengthDisplay, thread.length)));
            }
            return hiddenStackWarningDisplayed = true;
          }
        });
      }
    }
